        ...
        draw() {
            ...
            if (this.cling == 1) {
                ...
                if (this.carrying == 1 && this.clingTo.health <= 0) {
                    ...
                    // for(let t = 0;t<this.glop.neighbors.length;t++){

                    // }


                    if (this.glop == this.body || (Math.random() < .05)) { //.33
                        for (let t = 0; t < throbert.nodes.length; t++) {
                ...
                if(this.gloppath == 0){
                    ...
                    this.clingTo.xmom -= (((Math.cos(anfle) * this.clingTo.weight)) * .022) * (1 + (this.bloom * .1))
                    this.clingTo.ymom -= (((Math.sin(anfle) * this.clingTo.weight)) * .022) * (1 + (this.bloom * .1))
                    // linkus.draw()
                    this.gloppath = 1
                    }
                }else{
                ...
                if(this.boglpath == 0){
                    ...
                    let anfleg = linkg.angle()
                            this.clingTo.xmom -= (((Math.cos(anfleg) * this.clingTo.weight) * 1) * .06) * (1 + (this.bloom * .1))
                            this.clingTo.ymom -= (((Math.sin(anfleg) * this.clingTo.weight) * 1) * .06) * (1 + (this.bloom * .1))
                            this.boglpath = 1
                    }
                }else{
                    let linkg = new LineOP(this.clingTo, this.bogl, "pink", 2)
                    ...
                    // for (let p = 0; p < this.glop.paths.length; p++) {
                    //     if (this.glop.neighbors[p].priority < this.glop.priority) {
                    //         let linkus = new LineOP(this.clingTo, this.glop.neighbors[p])
                    //         let anfle = linkus.angle()
                    //         this.clingTo.xmom -= (((Math.cos(anfle) * this.clingTo.weight) * c1) * .06) * (1 + (this.bloom * .1))
                    //         this.clingTo.ymom -= (((Math.sin(anfle) * this.clingTo.weight) * c1) * .06) * (1 + (this.bloom * .1))
                    //         // this.glop.paths[p].color = "green"
                    //         // this.glop.paths[p].draw()
                    //     } else if (this.glop.neighbors[p].priority > this.glop.priority) {
                    //         // let linkus = new LineOP(this.glop.neighbors[p], this.glop)
                    //         // let anfle = linkus.angle()
                    //         // this.clingTo.xmom -= (((Math.cos(anfle) * this.clingTo.weight) * c2) * .016) * (1 + (this.bloom * .1))
                    //         // this.clingTo.ymom -= (((Math.sin(anfle) * this.clingTo.weight) * c2) * .016) * (1 + (this.bloom * .1))
                    ...
                    if (this.camplink.hypotenuse() < 280) {
                        ...
                        this.clingTo.x -= (Math.cos(angle) * this.clingTo.weight) * 5
                        this.clingTo.y -= (Math.sin(angle) * this.clingTo.weight) * 5
                        // t = 999999999999
                        // break
                    }
