...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000055") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            if (throbert.pause.paused == 1) {
                throbert.pause.buttonz(TIP_engine)
            }
        ...
        function continued_stimuli(e) {
            ...
            // throbert.guiding = 1
            if (throbert.qsmack == 1) {
                throbert.seekx = ((throbert.seekx * 3) + (throbert.seekz)) * .25
                throbert.seeky = ((throbert.seeky * 3) + (throbert.seekw)) * .25
            }
        }
    }
                    ...
                    if (this.clingTo.doesPerimeterTouch(throbert.c1)) {
                        ...
                        if (this.clingTo.timer > 20) {
                            ...
                            if (throbert.playlink.hypotenuse() < 900) {
                                absorb.volume = Math.max(.3 - (throbert.playlink.hypotenuse() * .0005), 0)
                                absorb.play()
                            }
                        }
                ...
                if (this.health > 0) {
                    if (this.health < this.maxhealth) {
                        ...
                        if ((this.cycle % 100).between(80, 100)) {
                            ...
                            this.pulse = 1
                            geooff.volume = Math.max(.6 - (this.playlink.hypotenuse() * .0005), 0)
                            geooff.play()
                        } else {
                            this.pulse = 0
                            this.body.radius = 40
    ...
    class Hoggelspouzer {
        ...
        draw() {
            ...
            if (this.health > 0) {
                if ((this.spoutcount % 100).between(80, 100) && this.spoutcount % 6 == 0) {
                    peh.volume = Math.max(.3 - (this.playlink.hypotenuse() * .0005), 0)
                    peh.play()
        ...
        draw() {
            ...
            if (this.health > 0) {
                if ((this.spoutcount % 50 == 44)) {
                    ...
                    bubpeh.volume = Math.max(.3 - (this.playlink.hypotenuse() * .0005), 0)
                    bubpeh.play()
                    this.shots.push(shot)
                }
            }
    ...
    const ploosheetx = new Image()
    ploosheetx.src = "ploosheetxt.png"
    ...
    class Exploorenab {
        constructor(x, y) {
            this.body = new Circle(x, y, 12, "pink")
            this.bodyarea = new Circle(x, y, 150, "pink")
            this.go = this.body
            this.health = 2000
            this.maxhealth = this.health
        ...
        draw() {
            ...
            //this.healthDraw()
            // this.body.draw()
            canvas_context.drawImage(ploosheetx, 24 * (this.step % 18), 0, 24, 24, this.body.x - this.body.radius, this.body.y - this.body.radius, 24, 24)
            this.body.health = this.health
    ...
    class PauseMenu {
        constructor() {
            ...
            this.paused = 0
            this.buttons = []
            this.buttons.push(new MenuButton(100, 100, "Return"))
            this.buttons.push(new MenuButton(100, 150, "Map"))
        ...
        buttonz(point) {
            ...
            for (let t = 0; t < this.buttons.length; t++) {
                if (this.buttons[t].body.isPointInside(point)) {
                    this.state = t
                    this.status = {}
                    this.states[this.state] = this.status
                    break
                }
            }
        }
            ...
            if (this.paused == 1) {
                ...
                if (this.state != -1) {
                    if (this.state == 0) {
                        ...
                        if (throbert.mode == 0) {
                            megamap_context.drawImage(throbert.worldmap, 0, 0, 5120, 5120, 0, 0, megamap.width, megamap.height) ///, 1024, 1024, 0, 0, 10240, 10240)
                        } else {
                            megamap_context.drawImage(throbert.worldmap, 0, 0, 1024, 1024, 0, 0, megamap.width, megamap.height) ///, 1024, 1024, 0, 0, 10240, 10240)
                        }
                        // megamap_context.drawImage(canvas, 0,0, 1280,720, (throbert.body.x + 200) + (Math.max(throbert.body.x-640, 0)/32), (throbert.body.y-200) + (Math.max(throbert.body.y-360, 0)/32), (1280/32),(720/32))
                        ...
                        canvas_context.font = "20px arial"
                        canvas_context.fillStyle = "white"
                        let seconds = Math.floor(Math.abs(oldnow - Date.now()) / 1000)
                        ...
                        canvas_context.fillText("Playtime: " + (days + ":" + hours + ":" + minutes + ":" + seconds), this.body.x + 750, this.body.y + 300)
            ...
            if (this.active == 1) {
                if (this.step == 7) {
                    if (this.body.radius < 51) {
                        boom.volume = Math.max(.3 - (this.playlink.hypotenuse() * .0005), 0)
                        boom.play()
                        this.body.radius += 8
                    } else {
                        this.spliceout = 1
    ...
    class Throbert {
        ...
        draw() {
            ...
            this.volumeplay = (sum / 3) * .3 //.5

            charge.volume = Math.min(this.volumeplay, .45)
            ...
            for (let t = 0; t < this.enemies.length; t++) {
                if (this.enemies[t].spliceout == 1) {
                    ...
                    if (this.enemies[t].collect == 1) {
                        throbert.parts++
                        throbert.partslog.push(this.enemies[t].type)
                    }
                    this.enemies.splice(t, 1)
                    if (govsl > 0) {
                        this.spawn(govsl)
            ...
            if (keysPressed['u'] || keysPressed['c'] || keysPressed['q'] || gamepadAPI.buttonsStatus.includes('Right-Trigger') || this.whistling == 1) {
                for (let t = 0; t < this.sproutventory.length; t++) {
            ...
            for (let t = 0; t < this.sproutventory.length; t++) {
                ...
                if (this.sproutventory[t].grab == 1) {
                    ...
                    if (keysPressed[' '] || gamepadAPI.buttonsStatus.includes('Left-Trigger')) {
                        if (keysPressed[' '] || Math.abs(gamepadAPI.axesStatus[2]) > 0.09 || Math.abs(gamepadAPI.axesStatus[3]) > 0.09) {
                            this.grab = -3
                            if (this.flingframes < 10) {
                                flingsound.volume = .5
                                flingsound.currentTime = Math.random() * .7 //*1.7
                                flingsound.play()
                                this.flingframes = 50
                            }
                            ...
                            if (this.sproutventory[t].type == 2) {
                                ...
                                if (this.sproutventory[t].carryingbomb == 1) {
                                    fwee.volume = .4
                                    fwee.play()
                                }
                            } else {
                ...
                if (keysPressed['o'] || keysPressed['v'] || keysPressed['e'] || gamepadAPI.buttonsStatus.includes('A') || gamepadAPI.buttonsStatus.includes('Left-Trigger')) {
    ...
    function main() {
        ...
        if (throbert.pause.paused == 1) {
            ...
            if (gamepadAPI.buttonsStatus.includes('Start') || keysPressed['p']) {
