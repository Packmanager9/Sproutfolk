...
function isNyaN(num){
    ...
    class Pearl {
        constructor(x,y, radius, color){
            ...
            this.points = []
            for(let t = 0;t<60;t++){
                this.distances.push(this.body.radius)
                let point = new Circle(this.body.x + (Math.cos(t*(Math.PI/30) * this.distances[t])), this.body.y + (Math.sin(t*(Math.PI/30) * this.distances[t])), 1, this.body.color)
        ...
        draw(){
            ...
            this.spin*=.99
            let links = new LineOP(this.body, loe.tongue[loe.index])
            this.body.xmom += -Math.cos(links.angle())*.1
            this.body.ymom += -Math.sin(links.angle())*.1
             links = new LineOP(this.body, loe.tongue[loe.index2])
            this.body.xmom += -Math.cos(links.angle())*.1
            this.body.ymom += -Math.sin(links.angle())*.1
            this.body.frictiveMove()
            this.points = []
            let poly = new Polygon(this.body.x, this.body.y, 100, "white", 40)
            for(let t = 0;t<60;t++){
                let point = new Circle(this.body.x + (Math.cos(this.angle+(t*(Math.PI/30))) * this.distances[t]), this.body.y +  (Math.sin(this.angle+(t*(Math.PI/30))) * this.distances[t]), 1, this.body.color)
                point.color = `rgb(255,${180+(t*3)},${180+(t*3)}`
                this.points.push(point)
            }
            poly.nodes = [...this.points]
            poly.draw()

            let link = new LineOP(this.points[this.points.length-1], this.points[Math.max(0,0)], `white`, 1)
            link.draw()
            for(let t = 0;t<this.points.length;t++){

            let link = new LineOP(this.points[t], this.points[Math.max(t-1,0)], `rgb(255,${180+(t*3)},${180+(t*3)}`, 1)
            link.draw()
            ...
            let wet = 0
            for(let t = 0;t<this.points.length;t++){
                if(this.points[t].doesPerimeterTouch(loe.shelltop)){
            ...
            if(wet == 1){
                let link = new LineOP(this.body, loe.shelltop)
                this.body.xmom += -Math.cos(link.angle())*20
                this.body.ymom += -Math.sin(link.angle())*20
            }
        }
    }
    ...
    class Clam {
        constructor(){
            this.tongue = []
            this.shelltop = new Circled(360, 360, 300, "salmon")
            for(let t = 0;t<250;t++){
                let point = new Circle((110+(.25*250))+(t*1.5), 360, 16, "red")
                this.tongue.push(point)
                this.index = 128
                this.index2 = 128
                this.pearl = new Pearl(350,300, 3, "white")
                this.pearl.body.xmom = 0
                this.pearl.body.ymom = 0
            }
        }
        draw(){
            this.shelltop.draw()
                ...
                if (keysPressed['d']) {
                 ...
                 if (keysPressed['l']) {
                     loe.index2+=5
                     loe.index2%=loe.tongue.length-1
                  }
                  if (keysPressed['j']) {
                      loe.index2-=5
                      if(loe.index2 < 0){
                      ...
                      loe.index2%=loe.tongue.length
                  }


                  control(this.tongue[this.index], 32)
                  // if(keysPressed[' ']){
                      control(this.tongue[Math.max(this.index-2,0)], 4)
                      control(this.tongue[Math.min(this.index+1, this.tongue.length-1)], 8)
                      control(this.tongue[Math.max(this.index-1,0)], 8)
                      control(this.tongue[Math.min(this.index+2, this.tongue.length-1)], 4)
                      control(this.tongue[Math.max(this.index-3,0)], 4)
                      control(this.tongue[Math.min(this.index+3, this.tongue.length-1)], 4)



                      dontrol(this.tongue[this.index2], 32)
                      dontrol(this.tongue[Math.max(this.index2-2,0)], 4)
                      dontrol(this.tongue[Math.min(this.index2+1, this.tongue.length-1)], 8)
                      dontrol(this.tongue[Math.max(this.index2-1,0)], 8)
                      dontrol(this.tongue[Math.min(this.index2+2, this.tongue.length-1)], 4)
                      dontrol(this.tongue[Math.max(this.index2-3,0)], 4)
                      dontrol(this.tongue[Math.min(this.index2+3, this.tongue.length-1)], 4)


                    dontrol
                // }
            for(let t =this.tongue.length-1; t>0;t--){
                if(t > 0 && t < this.tongue.length-1){
                    this.tongue[t].y = ((((this.tongue[t].y*1) + (this.tongue[t+1].y*9))*.1)+(((this.tongue[t].y*1) + (this.tongue[t-1].y*9))*.1))*.5
                }else{
                if(t > 0){
                    this.tongue[t].y = (((this.tongue[t].y*1) + this.tongue[t-1].y)*.5)
                }
                if(t < this.tongue.length-1){
                    this.tongue[t].y = (((this.tongue[t].y*1) + this.tongue[t+1].y)*.5)
                }
                }
            }
            ...
            for(let t = 0;t<this.tongue.length;t++){
                if(t > 0 && t < this.tongue.length-1){
                    this.tongue[t].y = ((((this.tongue[t].y*1) + (this.tongue[t+1].y*9))*.1)+(((this.tongue[t].y*1) + (this.tongue[t-1].y*9))*.1))*.5
                }else{
                if(t > 0){
            ...
            for(let t = 0;t<this.tongue.length;t++){
                ...
                this.tongue[t].radius = 16
                this.tongue[this.index].radius = 18
                // if(t == this.index){
                //     this.tongue[t].color = "magenta"
                // }else{
                //     this.tongue[t].color = "pink"
                // }
                if(t > 0 && t < this.tongue.length-1){
                    this.tongue[t].y = ((((this.tongue[t].y*1) + this.tongue[t+1].y)*.5)+(((this.tongue[t].y*1) + this.tongue[t-1].y)*.5))*.5
                    if(t-1 > 0 && t < this.tongue.length-2){
                        this.tongue[t].y = ((((this.tongue[t].y*1) + this.tongue[t+2].y)*.5)+(((this.tongue[t].y*1) + this.tongue[t-2].y)*.5))*.5
                    }
                    // let link  = new LineOP(this.tongue[t-1], this.tongue[t+1], "#FFAAAA66", 10)
                    // link.draw()
                }else{
                if(t > 0){
                    this.tongue[t].y = (((this.tongue[t].y*1) + this.tongue[t-1].y)*.5)
                ...
                if(this.tongue[t].y > 360){
                    this.tongue[t].y-=.25
                    if(t == 0 || t == this.tongue.length-1){
                        this.tongue[t].y-= (this.tongue[t].y-360)/10
                    }
                }
                if(this.tongue[t].y > 460){
                    this.tongue[t].y-= (this.tongue[t].y-360)/64
                    if(t == 0 || t == this.tongue.length-1){
                        this.tongue[t].y-= (this.tongue[t].y-360)/16
                    }
                }
                if(this.tongue[t].y < 360){
                    this.tongue[t].y+=.25
                ...
                for(let k = 0;k<this.pearl.points.length;k++){
                    if(this.pearl.points[k].doesPerimeterTouch(this.tongue[this.index])){
                        let link = new LineOP(this.pearl.body, this.tongue[this.index])
                        let ang = link.angle()
                        let tryindex = Math.abs(Math.floor(ang+Math.PI/(Math.PI/60)))
                        this.pearl.body.xmom = Math.cos(ang)* this.pearl.distances[(tryindex+0)%this.pearl.distances.length]*.5
                        this.pearl.body.ymom = Math.sin(ang)*  this.pearl.distances[(tryindex+0)%this.pearl.distances.length]*.5
                        this.pearl.spin = (this.pearl.angle-ang)/30
                        for(let r = k-12;r<k+12;r++){
                            if( this.pearl.distances[(tryindex+r)%this.pearl.distances.length] < 22){
                                this.pearl.distances[(tryindex+r)%this.pearl.distances.length]+=.000008*(Math.abs((k-r)-8))
                            }
                        }
                    }
                    if(this.pearl.points[k].doesPerimeterTouch(this.tongue[this.index2])){
                        let link = new LineOP(this.pearl.body, this.tongue[this.index])
                        let ang = link.angle()
                        let tryindex = Math.abs(Math.floor(ang+Math.PI/(Math.PI/60)))
                        this.pearl.body.xmom = Math.cos(ang)* this.pearl.distances[(tryindex+0)%this.pearl.distances.length]*.5
            ...
            for(let k = 0;k<this.pearl.points.length;k++){
                if(this.pearl.points[k].doesPerimeterTouch(this.tongue[t])){
                    let link = new LineOP(this.pearl.body, this.tongue[t])
                    let ang = link.angle()
                    let tryindex = Math.abs(Math.floor(ang+Math.PI/(Math.PI/120)))
                    this.pearl.body.xmom = Math.cos(ang)* this.pearl.distances[(tryindex+0)%this.pearl.distances.length]*.15
                    this.pearl.body.ymom = Math.sin(ang)*  this.pearl.distances[(tryindex+0)%this.pearl.distances.length]*.15
                    this.pearl.spin = 0 // (this.pearl.angle-ang)/20

                    // console.log(tryindex)
                    for(let r = k-18;r<k+18;r++){
                        if( this.pearl.distances[(tryindex+r)%this.pearl.distances.length] < 22){
                            this.pearl.distances[(tryindex+r)%this.pearl.distances.length]+=.0000008*(Math.abs((k-r)-8))
                        }
                    }
        ...
        for(let t = 0;t<this.tongue.length;t++){
            if(t == this.index){
                this.tongue[t].color = "magenta"
            }else if(t == this.index2){
                this.tongue[t].color = "salmon"
            }else{
                this.tongue[t].color = "pink"
            }
            if(t > 0 && t < this.tongue.length-1){
                let link  = new LineOP(this.tongue[t-1], this.tongue[t+1], "#FFAAAA66", 32)
                link.draw()
            }else{
            }
        }
        ...
        this.tongue[0].draw()
        this.tongue[this.tongue.length-1].draw()
        this.tongue[this.index].draw()
        this.tongue[this.index2].draw()
        }
    }

    let loe = new Clam()

    let colors = ['#FF0000', '#FF8800', '#Ffff00', '#88ff00', '#00ff00', '#00ff88', '#00ffff', '#0088ff', '#0000ff', '#8800ff', '#ff0088']

    class Hall {
        constructor(){
            this.center = new Point(360,360)
            this.dots = [[], [], [], []]
            this.coefficients = [Math.random()*5, Math.random()*5, Math.random()*5, Math.random()*5]
            this.count = -1
            this.angle = 0
        }
        draw(){
            ...
            for(let t = 0;t<this.dots.length;t++){
                this.dots[t].push(new Circle(this.center.x, this.center.y, (this.coefficients[t])*2, "red",Math.sign(( t%2)-.5),Math.sign(( (t+1)%2)-.5)))
            }
            this.count++
            if(this.count > (360/5)){
            ...
            for(let t = this.count;t<this.dots[0].length;t++){
                this.dots[0][t].x = this.center.x - t*this.coefficients[0]
                this.dots[0][t].y = this.center.y - t*this.coefficients[0]
                this.dots[0][t].color = colors[t%colors.length]
                this.dots[0][t].draw()
            }
            for(let t = this.count;t<this.dots[1].length;t++){
                // this.dots[1][t].move()
                this.dots[1][t].x = this.center.x + t*this.coefficients[1]
                this.dots[1][t].y = this.center.y - t*this.coefficients[1]
                this.dots[1][t].color = colors[t%colors.length]
                this.dots[1][t].draw()
            }
            for(let t = this.count;t<this.dots[2].length;t++){
                // this.dots[2][t].move()
                this.dots[2][t].x = this.center.x + t*this.coefficients[2]
                this.dots[2][t].y = this.center.y + t *this.coefficients[2]
                this.dots[2][t].color = colors[t%colors.length]
                this.dots[2][t].draw()
            }
            for(let t = this.count;t<this.dots[3].length;t++){
                // this.dots[3][t].move()
                this.dots[3][t].x = this.center.x - t*this.coefficients[3]
                this.dots[3][t].y = this.center.y + t*this.coefficients[3]
                this.dots[3][t].color = colors[t%colors.length]
                this.dots[3][t].draw()
            }
            for(let t = this.count;t<this.dots[0].length;t++){
                let link = new LineOP(this.dots[1][t], this.dots[2][t], colors[t%colors.length], 5)
                link.draw()
            }
            for(let t = this.count;t<this.dots[0].length;t++){
                let link = new LineOP(this.dots[0][t], this.dots[1][t], colors[t%colors.length], 5)
                link.draw()
            }
            for(let t = this.count;t<this.dots[0].length;t++){
                let link = new LineOP(this.dots[2][t], this.dots[3][t], colors[t%colors.length], 5)
                link.draw()
            }
                for(let t = this.count;t<this.dots[0].length;t++){
                let link = new LineOP(this.dots[3][t], this.dots[0][t], colors[t%colors.length], 5)
                link.draw()
            }
        }
    ...
    //     while(cards[index] != 5 && (cards.includes(1) || cards.includes(2) || cards.includes(3) || cards.includes(4))){
    ...
    //         }
    //     }
    //     if(!(cards.includes(1) || cards.includes(2) || cards.includes(3) || cards.includes(4))){
    //         count++
    //     }
    // }
    // console.log(count, jokercount, total, (count/total))
