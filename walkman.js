...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#FFFFFF") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            for(let t = 0;t<walkmin.nodes.length;t++){
                if(walkmin.nodes[t].isPointInside(TIP_engine)){
    ...
    class Walkman {
        constructor(x,y, size, colors){
            ...
            for(let t = 0;t<17;t++){
                let arm = new Circle(this.body.x+(size+(size/16))  +( (t/9)*size), this.body.y +( t*(size*(.052+(t/111)))), (size-((size/2)*(t/16)))*.5, colors[((t+1)%2)])
                this.nodes.push(arm)
            }
            for(let t = 0;t<17;t++){
                let arm = new Circle((this.body.x-(size+(size/16)))  -( (t/9)*size), this.body.y +( t*(size*(.052+(t/111)))), (size-((size/2)*(t/16)))*.5, colors[((t+1)%2)])
            ...
            for(let t = 0;t<42;t++){
                let arm = new Circle(this.body.x+(size*.1)+( (t/22)*size), this.body.y +( t*(size*(.09+(t/1150)))) + lheight, (size-((size/6)*(t/20)))*.5, colors[((t+1)%2)])
                this.nodes.push(arm)
                let arm2 = new Circle(this.body.x-(size*.1) -( (t/22)*size), this.body.y +( t*(size*(.09+(t/1150))))+ lheight, (size-((size/6)*(t/20)))*.5, colors[((t+1)%2)])
            ...
            for(let t = 1;t<this.nodes.length;t++){
                ...
                this.nodes[t].dis = link.hypotenuse()*.8
        ...
        draw(){
            ...
            canvas_context.fillText("$"+Math.floor(this.money*100)/100, 10,40)
            control(this.body, 3)
            if(keysPressed[' ']){
            ...
            for(let t = 1;t<this.nodes.length;t++){
                ...
                if(this.blast <= 0){
                    this.nodes[t].x = ((((((this.nodes[t].x*this.nodes[t].dis) + ((((Math.cos(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.x)))*10))/(this.nodes[t].dis+10))*10)  +  ((((Math.cos(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.x))))  /11
                    this.nodes[t].y = ((((((this.nodes[t].y*this.nodes[t].dis) + ((((Math.sin(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.y)))*10))/(this.nodes[t].dis+10))*10)   +   ((((Math.sin(this.nodes[t].ang-Math.PI)*this.nodes[t].dis))+(this.body.y))) )/11
                }    
                ...
                if(this.nodes[t].x+this.nodes[t].radius > canvas.width){
                    ...
                    if(this.nodes[t].xmom > 0){
                        this.nodes[t].xmom*=-.8
                    }
                    if(this.nodes[t].skull == 1){
                        this.money += Math.abs(this.nodes[t].ymom)+Math.abs(this.nodes[t].xmom)
                    }
                }
                if(this.nodes[t].y+this.nodes[t].radius > canvas.height){
                    this.nodes[t].y = canvas.height-this.nodes[t].radius
                    if(this.nodes[t].ymom > 0){
                        this.nodes[t].ymom*=-.8
                    }
                    if(this.nodes[t].skull == 1){
                        this.money += Math.abs(this.nodes[t].ymom)+Math.abs(this.nodes[t].xmom)
    ...
    let walkmin = new Walkman(350,350, 13, ["yellow", "black"])
    function main() {
